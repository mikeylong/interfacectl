 ...
 */
export function detectDiffNoise(entries) {
 ...
 */
export function validatePolicyVersion(policy) {
    ...
    return (policy.fingerprint !== undefined &&
 ...
 */
export function detectContractEvolution(oldContract, newContract) {
 ...
 */
export function validateObservedStability(descriptor) {
    // Check for required fields
    if (!descriptor.surfaceId || descriptor.surfaceId.length === 0) {
        return false;
    }
    // Check that arrays are arrays
    if (!Array.isArray(descriptor.sections))
        return false;
    if (!Array.isArray(descriptor.fonts))
        return false;
    if (!Array.isArray(descriptor.colors))
        return false;
    if (!Array.isArray(descriptor.motion))
        return false;
 ...
 */
export function detectRenameInflation(oldPaths, newPaths) {
 ...
 */
export function validateOutputSchema(output, schemaVersion) {
    return output.schemaVersion === schemaVersion;
}
/**
 * 9. Severity inflation: Policy should control thresholds, not hardcoded
 */
export function checkSeverityInflation(entries, policy) {
    ...
    const hasSeverityOverrides = policy.autofixRules.some((rule) => rule.setSeverity !== undefined);
    // If many rules set severity to "error", it might be inflation
    const errorOverrides = policy.autofixRules.filter((rule) => rule.setSeverity === "error");
 ...
 */
export function ensureReproCommand(output, workspaceRoot) {
    if (!output.repro) {
 ...
 */
export function detectAllDriftRisks(diff, policy) {
    const risks = [];
    // 1. Diff noise
    ...
    const entriesWithoutRules = diff.entries.filter((e) => !validateSemanticClarity(e));
